    ...
    if (!sv_csrf_valid('ai-image-validation', $_POST['csrf_token'] ?? null)) {
        ...
        if ($stagingId <= 0 || !is_array($row) || !in_array($action, ['publish', 'reject', 'regenerate'], true)) {
            ...
            try {
                ...
                if ($targetChannel === '') $targetChannel = strtolower(trim((string)($_POST['channel'] ?? 'site')));
                ...
                $flashMessage = "Imagem #{$stagingId} regenerada para {$channelLabels[$targetChannel]} com a foto real e as regras do canal reaplicadas.";
            ...
            $channel = $intended !== '' ? $intended : strtolower(trim((string)($_POST['channel'] ?? '')));
            if (!isset($channelLabels[$channel]) || ($_POST['confirm_channel'] ?? '') !== $channel) {
                $flashError = 'Confirme explicitamente o marketplace de destino desta imagem.';
            } elseif ($intended !== '' && $channel !== $intended) {
                $flashError = 'Esta imagem foi gerada para outro marketplace. Regere para o destino correto antes de publicar.';
            } else {
                try {
...
<div class="note">O destino escolhido na geracao fica preso ao staging. Para publicar em outro marketplace, gere/regere a imagem com as regras daquele canal. A white continua obrigatoria primeiro nos marketplaces externos configurados.</div>
...
<article class="card"><div class="top"><h2><?=ais_v_h((string)($item['product_name']?:'Produto #'.$item['product_id']))?></h2><span class="badge"><?=ais_v_h((string)($channelLabels[$defaultChannel]??$defaultChannel))?></span></div><div class="meta">SKU <?=ais_v_h((string)($item['product_sku']??''))?> · tipo <?=ais_v_h((string)$item['image_type'])?> · provedor <?=ais_v_h((string)$item['provider_used'])?> · <span class="status"><?=ais_v_h((string)$item['status'])?></span></div>
<div class="profile"><strong>Auditoria visual de <?=ais_v_h((string)$profile['label'])?></strong><ul><?php foreach((array)($profile['audit_notes']??[]) as $note):?><li><?=ais_v_h((string)$note)?></li><?php endforeach;?></ul><?php if($w>0):?><div class="<?=$meetsRec?'quality-ok':'quality-warn'?>">Arquivo gerado: <?=$w?>x<?=$h?>. Alvo recomendado: <?=$recommended?>x<?=$recommended?>. <?=$meetsRec?'Atende o alvo recomendado.':'Publicacao pode ser tecnicamente valida, mas esta abaixo do alvo recomendado e deve ser revisada.'?></div><?php endif;?></div>
<div class="compare"><div><h3>Antes — foto real</h3><div class="image-box"><?php if(trim((string)$item['original_image'])!==''):?><img src="<?=ais_v_h((string)$item['original_image'])?>" alt="Foto real do produto"><?php else:?><p>Produto sem foto original disponivel.</p><?php endif;?></div></div><div><h3>Depois — gerada para <?=ais_v_h((string)$profile['label'])?></h3><div class="image-box"><img src="<?=ais_v_h((string)$item['local_path'])?>?v=<?=urlencode((string)$item['updated_at'])?>" alt="Imagem gerada"></div></div></div>
<?php if((string)($item['error_message']??'')!==''):?><div class="alert err"><?=ais_v_h((string)$item['error_message'])?></div><?php endif;?>
<form method="post" class="edit"><input type="hidden" name="csrf_token" value="<?=ais_v_h($csrf)?>"><input type="hidden" name="staging_id" value="<?=(int)$item['id']?>"><input type="hidden" name="channel" value="<?=ais_v_h($defaultChannel)?>"><h3>Regenerar mantendo produto + regras do canal</h3><label>Instrucao adicional de cena<textarea name="prompt_used" rows="5" required><?=ais_v_h((string)$item['prompt_used'])?></textarea></label><label>Motor<select name="regeneration_engine"><option value="openai">OpenAI — edicao da foto real</option><option value="google">Gemini — edicao da foto real</option><option value="claude">Claude otimiza cena + OpenAI edita</option></select></label><div class="actions"><button class="regenerate" name="action" value="regenerate" formnovalidate>Regenerar para <?=ais_v_h((string)$profile['label'])?></button></div>
<h3>Publicacao real</h3><?php if($legacy):?><div class="alert warn">Item legado sem marketplace persistido. O destino abaixo sera gravado ao regenerar; prefira regenerar antes de publicar.</div><label>Canal<select name="channel"><?php foreach($channelLabels as $key=>$label):?><option value="<?=ais_v_h($key)?>"><?=$key===$defaultChannel?'selected':''?>><?=ais_v_h($label)?></option><?php endforeach;?></select></label><?php else:?><p>Destino bloqueado no staging: <strong><?=ais_v_h((string)$profile['label'])?></strong>.</p><?php endif;?>
<?php $reg=$endpointRegistry[$defaultChannel]??null;if(is_array($reg)):?><details class="endpoints"><summary>Endpoints de imagem e read-back</summary><?php foreach(array_merge($reg['images'],$reg['readback']) as $endpoint):?><code><?=ais_v_h((string)$endpoint)?></code><?php endforeach;?></details><?php endif;?><label class="confirm"><input type="checkbox" name="confirm_channel" value="<?=ais_v_h($defaultChannel)?>" required> Confirmo atualizar somente <strong><?=ais_v_h((string)$profile['label'])?></strong>.</label><div class="actions"><button class="publish" name="action" value="publish">Aprovar e publicar</button><button class="reject" name="action" value="reject" formnovalidate>Rejeitar sem publicar</button></div></form></article><?php endforeach;?></section></main></body></html>
