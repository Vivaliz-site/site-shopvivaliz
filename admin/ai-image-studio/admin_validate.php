...
$channelLabels = ['site'=>'ShopVivaliz','ml'=>'Mercado Livre','shopee'=>'Shopee','amazon'=>'Amazon','tiktok'=>'TikTok Shop','erp'=>'Olist / Tiny ERP'];
...
/** @return array{name:string,description:string,image_ref:string}|null */
    ...
    if (!sv_csrf_valid('ai-image-validation', $_POST['csrf_token'] ?? null)) {
        ...
        if ($stagingId <= 0 || !is_array($row) || !in_array($action, ['publish', 'reject', 'regenerate'], true)) {
            ...
            try {
                ...
                $prompt = trim((string)($_POST['prompt_used'] ?? ''));
                if ($prompt === '') throw new RuntimeException('Informe o prompt antes de regenerar.');
                $engine = strtolower(trim((string)($_POST['regeneration_engine'] ?? 'openai')));
                if (!in_array($engine, ['openai', 'google', 'claude'], true)) throw new RuntimeException('Motor de regeneração inválido.');
                ...
                if ($engine === 'google') {
                    ...
                    if ($engine === 'claude') {
                        $prompts = (new AiStudioClaudeClient(AI_STUDIO_CLAUDE_API_KEY, AI_STUDIO_CLAUDE_MODEL))->optimizePrompts(
                            $product['name'],
                            $product['description'] . "\nInstrução adicional do administrador: " . $prompt
                        );
                        $prompt = (string)($prompts[$imageType] ?? $prompt);
                ...
                if (!is_file($destination) || (int)filesize($destination) <= 0) throw new RuntimeException('A API não gerou um arquivo de imagem válido.');
                ...
                $flashMessage = "Imagem #{$stagingId} regenerada de verdade. Compare novamente antes de aprovar.";
            ...
            if (!isset($channelLabels[$channel]) || ($_POST['confirm_channel'] ?? '') !== $channel) {
                $flashError = 'Selecione e confirme explicitamente um único canal.';
            } else {
                try {
...
<div class="note">Cada aprovação atualiza somente um canal. Antes de aprovar, você pode editar o prompt e regenerar a imagem usando a foto real do produto como referência. Preço e estoque nunca entram no payload.</div>
...
<form method="post" class="edit"><input type="hidden" name="csrf_token" value="<?=ais_v_h($csrf)?>"><input type="hidden" name="staging_id" value="<?=(int)$item['id']?>"><h3>Alterar a geração antes de aprovar</h3><label>Prompt editável<textarea name="prompt_used" rows="5" required><?=ais_v_h((string)$item['prompt_used'])?></textarea></label><label>Motor para regenerar<select name="regeneration_engine"><option value="openai">OpenAI — edição da foto real</option><option value="google">Gemini — edição da foto real</option><option value="claude">Claude otimiza o prompt + OpenAI edita</option></select></label><div class="actions"><button class="regenerate" name="action" value="regenerate" formnovalidate>Regenerar e revisar novamente</button></div>
<h3>Aprovação por canal único</h3><label>Canal<select name="channel" class="channel-select"><?php foreach($channelLabels as $key=>$label):?><option value="<?=ais_v_h($key)?>" <?=$key===$defaultChannel?'selected':''?>><?=ais_v_h($label)?></option><?php endforeach;?></select></label><div class="channel-endpoints"><?php foreach($endpointRegistry as $key=>$registry):?><div data-channel="<?=ais_v_h($key)?>" style="display:<?=$key===$defaultChannel?'block':'none'?>"><details class="endpoints"><summary>Endpoints de imagem e read-back</summary><?php foreach(array_merge($registry['images'],$registry['readback']) as $endpoint):?><code><?=ais_v_h($endpoint)?></code><?php endforeach;?></details></div><?php endforeach;?></div><label class="confirm"><input type="checkbox" name="confirm_channel" value="<?=$defaultChannel?>" class="confirm-channel" required> Confirmo atualizar somente o canal selecionado.</label><div class="actions"><button class="publish" name="action" value="publish">Aprovar e publicar no canal selecionado</button><button class="reject" name="action" value="reject" formnovalidate>Rejeitar sem publicar</button></div></form></article><?php endforeach;?></section></main>
