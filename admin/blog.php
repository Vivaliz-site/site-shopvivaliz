...
<div class="actions"><button type="submit">Salvar artigo</button> <?php if ($article): ?><a class="btn" href="/blog/<?= rawurlencode((string)$article['slug']) ?>" target="_blank">Abrir página pública</a><?php endif; ?> <a class="btn" href="/admin/blog.php">Novo</a></div>
</form></section>
<section class="card"><h2>Artigos</h2><div style="overflow:auto"><table><thead><tr><th>Título</th><th>Categoria</th><th>Status</th><th>Agendamento</th><th>Atualização</th><th></th></tr></thead><tbody><?php while($row=$list->fetch_assoc()): ?><tr><td><?= ab_esc((string)$row['title']) ?><br><small><?= ab_esc((string)$row['slug']) ?></small></td><td><?= ab_esc((string)$row['category']) ?></td><td><?= ab_esc((string)$row['status']) ?></td><td><?= ab_esc((string)($row['scheduled_at'] ?? '')) ?></td><td><?= ab_esc((string)$row['updated_at']) ?></td><td><a href="?id=<?= (int)$row['id'] ?>">Editar</a></td></tr><?php endwhile; ?></tbody></table></div></section>
</main></body></html>
