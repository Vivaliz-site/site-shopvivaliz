...
function cat_channel_before(PDO $db, int $productId, string $channel, array $local): array
{
    ...
    try {
        ...
        if ($channel === 'ml') {
            if ($externalId === '') {
                return array_merge($fallback, ['warning' => 'Mapeamento do anúncio Mercado Livre ainda não confirmado.']);
            }
        ...
        if ($channel === 'shopee') {
            if ($externalId === '') {
                return array_merge($fallback, ['warning' => 'Mapeamento do item Shopee ainda não confirmado.']);
            }
            ...
            $response = $client->request(
                ...
                null,
                ['item_id_list' => $externalId, 'need_complaint_policy' => 'false']
            );
        ...
        if ($channel === 'amazon') {
            ...
            $attributes = is_array($response['data']['attributes'] ?? null)
        ...
        if ($channel === 'tiktok') {
            if ($externalId === '') {
                return array_merge($fallback, ['warning' => 'Mapeamento do produto TikTok Shop ainda não confirmado.']);
            }
            $client = new SvTikTokClient();
            $response = $client->request('GET', '/product/202309/products/' . rawurlencode($externalId));
            $product = is_array($response['data']['product'] ?? null)
        ...
        if ($channel === 'erp') {
            ...
            if ($externalId === '' || !svtop_tiny_credentials_configured()) {
                return array_merge($fallback, ['warning' => 'ID ou OAuth Olist/Tiny indisponível para consulta oficial.']);
            }
            $token = svtop_tiny_get_token();
            $response = svtop_tiny_request('GET', '/produtos/' . rawurlencode($externalId), $token);
            $product = is_array($response['json']['produto'] ?? null)
        ...
        return array_merge($fallback, [
            'warning' => 'Não foi possível consultar o antes no canal: ' . $exception->getMessage(),
        ]);
    }
    ...
    if (!sv_csrf_valid('catalog-optimization', $_POST['csrf_token'] ?? null)) {
        ...
        if ($stagingId <= 0 || !is_array($row) || !in_array($action, ['publish', 'save', 'reject', 'regenerate'], true)) {
            ...
            try {
                ...
                if ($action === 'save') {
                    $flashMessage = "Alterações do conteúdo #{$stagingId} salvas em staging sem publicar. Revise e aprove quando estiver pronto.";
                } else {
                ...
                $flashError = $action === 'save'
                    ? 'As alterações não foram salvas: ' . $exception->getMessage()
                    : 'A publicação real não foi confirmada: ' . $exception->getMessage();
            }
        }
    }
}
...
<h2>Antes e depois — revise, altere, salve, regenere ou aprove</h2>
<?php if ($items === []): ?><div class="panel">Nenhum conteúdo aguardando decisão.</div><?php endif; ?>
<?php foreach ($items as $item):
    $pid = (int) $item['product_id'];
    $channel = (string) $item['channel'];
    $local = cat_original($db, $pid);
    $before = cat_channel_before($db, $pid, $channel, $local);
    $label = $channels[$channel] ?? $channel;
    $bullets = json_decode((string) $item['bullet_points_json'], true);
...
?>
<article class="item">
<div><span class="badge"><?= cat_h($label) ?></span> Produto #<?= $pid ?> · SKU <?= cat_h((string) $local['sku']) ?> · status <?= cat_h((string) $item['status']) ?></div>
