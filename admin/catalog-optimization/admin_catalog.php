...
<section class="panel"><h2>Gerar somente produtos ativos selecionados</h2><p>Busque na listagem, marque os produtos desejados e gere apenas os escolhidos. Produtos inativos ficam bloqueados no backend.</p><div class="controls"><label>Provedor<select id="provider"><option value="openai">OpenAI</option><option value="gemini">Gemini</option><option value="claude">Claude</option></select></label><label>Canal<select id="channel"><?php foreach($channels as $key=>$label):?><option value="<?=cat_h($key)?>"><?=cat_h($label)?></option><?php endforeach;?></select></label><label>Categoria<select id="category-filter"><option value="">Todas</option></select></label><label>Ordenar<select id="sort-by"><option value="relevance">Relevância</option><option value="name">Nome</option><option value="price-asc">Menor preço</option><option value="price-desc">Maior preço</option></select></label><label>Busca<input id="product-search" type="search" placeholder="SKU, nome, categoria..." autocomplete="off"></label><label>Itens por página<input id="page-size" type="number" min="6" max="48" value="12"></label><div class="actions"><button id="refresh-list" type="button">Atualizar lista</button><button id="select-page" type="button">Selecionar visíveis</button><button id="clear-selection" type="button">Limpar seleção</button><button id="run-selected" type="button">Gerar selecionados</button><button id="retry" type="button">Reprocessar falhas</button></div></div><div class="panel" style="margin-top:12px"><div class="source-title"><strong>Produtos ativos disponíveis</strong><small id="product-summary">Carregando...</small></div><div id="active-products-list" class="active-products-grid"></div><div class="actions" style="margin-top:12px;justify-content:space-between"><button id="prev-page" type="button">← Anterior</button><div id="page-status"></div><button id="next-page" type="button">Próxima →</button></div></div><pre id="log" class="log"></pre></section>
...
<script>(()=>{'use strict';const log=document.getElementById('log'),write=t=>{log.style.display='block';log.textContent+=t+'\n';log.scrollTop=log.scrollHeight};const $=id=>document.getElementById(id);const state={products:[],selected:new Set(),page:1,totalPages:1,query:'',pageSize:12,loading:false,categories:[]};const listEl=$('active-products-list'),summaryEl=$('product-summary'),pageStatusEl=$('page-status'),categoryFilterEl=$('category-filter'),sortByEl=$('sort-by');async function optimize(id,channel,provider){const r=await fetch('/admin/catalog-optimization/api/optimize_catalog.php',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({product_id:id,target_channel:channel,provider})});const d=await r.json();if(!r.ok&&!d.error)d.error='HTTP '+r.status;return d}function renderSelectedCount(){const count=state.selected.size;summaryEl.textContent=count===0?`${state.products.length} ativo(s) carregado(s) nesta página`:`${count} selecionado(s) · ${state.products.length} ativo(s) na página`;}function renderCategories(categories){const current=categoryFilterEl.value;categoryFilterEl.innerHTML=['<option value="">Todas</option>'].concat(categories.map(cat=>`<option value="${escapeHtml(cat)}">${escapeHtml(cat)}</option>`)).join('');if(categories.includes(current))categoryFilterEl.value=current;}function renderList(){if(!state.products.length){listEl.innerHTML='<div class="empty-state">Nenhum produto ativo encontrado nesta busca.</div>';pageStatusEl.textContent='';renderSelectedCount();return}listEl.innerHTML=state.products.map(product=>{const id=String(product.id||'');const checked=state.selected.has(id)?'checked':'';const price=Number(product.price||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});const stock=Number(product.stock||0);const meta=[product.sku?`SKU ${escapeHtml(product.sku)}`:'',product.category?escapeHtml(product.category):'',product.olist_product_id?`ID ${escapeHtml(product.olist_product_id)}`:''].filter(Boolean).join(' · ');return `<label class="product-card"><input type="checkbox" class="product-check" data-product-id="${escapeHtml(id)}" ${checked}><div><div class="product-card-title">${escapeHtml(product.name||'Produto sem nome')}</div><div class="product-card-meta">${meta}</div><div class="product-card-stats"><span>${price}</span><span>Estoque ${stock}</span><span>Ativo</span></div></div></label>`}).join('');listEl.querySelectorAll('.product-check').forEach(input=>{input.addEventListener('change',e=>{const id=String(e.currentTarget.dataset.productId||'');if(e.currentTarget.checked)state.selected.add(id);else state.selected.delete(id);renderSelectedCount()})});pageStatusEl.textContent=`Página ${state.page} de ${state.totalPages}`;renderSelectedCount()}function escapeHtml(value){const div=document.createElement('div');div.textContent=String(value??'');return div.innerHTML}async function loadProducts(page=1){state.loading=true;state.page=page;const query=$('product-search').value.trim();const pageSize=Math.max(6,Math.min(48,parseInt($('page-size').value||'12',10)||12));state.pageSize=pageSize;state.query=query;summaryEl.textContent='Carregando produtos ativos...';try{const url=new URL('/api/catalog/products.php',window.location.origin);url.searchParams.set('page',String(page));url.searchParams.set('limit',String(pageSize));const sortValue=sortByEl.value||'name';url.searchParams.set('sort',sortValue);if(query)url.searchParams.set('q',query);const category=categoryFilterEl.value.trim();if(category)url.searchParams.set('category',category);const r=await fetch(url.toString(),{credentials:'same-origin',cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||('HTTP '+r.status));state.products=Array.isArray(d.products)?d.products:[];state.totalPages=Math.max(1,Number(d.total_pages||1));state.categories=Object.keys(d.categories||{});renderCategories(state.categories);pageStatusEl.textContent=`Página ${Number(d.page||page)} de ${state.totalPages}`;renderList()}catch(err){listEl.innerHTML='<div class="empty-state">Erro ao carregar a lista de produtos ativos.</div>';summaryEl.textContent='Falha no carregamento';pageStatusEl.textContent='';write('ERRO: '+err.message)}finally{state.loading=false}}async function generateSelected(){const channel=$('channel').value;const provider=$('provider').value;const selectedIds=[...state.selected].map(id=>parseInt(id,10)).filter(Number.isFinite);if(!selectedIds.length){write('Nenhum produto selecionado.');return}const btn=$('run-selected');btn.disabled=true;log.textContent='';try{for(const id of selectedIds){const out=await optimize(id,channel,provider);write(`#${id}: ${out.success?'gerado':'falhou'} ${out.error||''}`)}await loadProducts(state.page)}finally{btn.disabled=false}}document.getElementById('run-selected').addEventListener('click',generateSelected);document.getElementById('refresh-list').addEventListener('click',()=>loadProducts(1));document.getElementById('select-page').addEventListener('click',()=>{state.products.forEach(product=>{const id=String(product.id||'');if(id)state.selected.add(id)});renderList()});document.getElementById('clear-selection').addEventListener('click',()=>{state.selected.clear();renderList()});document.getElementById('prev-page').addEventListener('click',()=>{if(state.page>1)loadProducts(state.page-1)});document.getElementById('next-page').addEventListener('click',()=>{if(state.page<state.totalPages)loadProducts(state.page+1)});document.getElementById('retry').addEventListener('click',async e=>{const b=e.currentTarget;b.disabled=true;log.textContent='';try{const r=await fetch('?ajax=failed_items',{credentials:'same-origin'}),d=await r.json();for(const item of(d.items||[])){const out=await optimize(item.product_id,item.channel,item.provider_used);write(`#${item.product_id}: ${out.success?'reprocessado':'falhou'} ${out.error||''}`)}await loadProducts(state.page)}catch(err){write('ERRO: '+err.message)}finally{b.disabled=false}});$('product-search').addEventListener('input',()=>loadProducts(1));$('page-size').addEventListener('change',()=>loadProducts(1));sortByEl.addEventListener('change',()=>loadProducts(1));categoryFilterEl.addEventListener('change',()=>loadProducts(1));document.querySelectorAll('[data-counter]').forEach(el=>{const target=el.closest('label')?.querySelector(`[data-count-for="${el.dataset.counter}"]`);if(!target)return;const update=()=>{target.textContent=`${el.value.length} caracteres${el.maxLength>0?' / '+el.maxLength:''}`};el.addEventListener('input',update);update()});loadProducts(1)})();</script></body></html>
