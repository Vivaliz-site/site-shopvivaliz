...
$catalog=json_decode((string)@file_get_contents(dirname(__DIR__).'/catalog/fallback-products.json'),true);$bySku=[];foreach(is_array($catalog)?$catalog:[] as $row){if(is_array($row)&&trim((string)($row['sku']??''))!=='')$bySku[trim((string)$row['sku'])]=$row;}
