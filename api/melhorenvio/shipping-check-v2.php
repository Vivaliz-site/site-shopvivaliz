...
$selected=$options[0]; $expiresAt=time()+1800; $fingerprint=['cep'=>$cep,'items'=>$fingerprintItems,'service_id'=>(string)$selected['id'],'price'=>round((float)$selected['price'],2),'expires_at'=>$expiresAt]; $quoteId=hash_hmac('sha256',json_encode($fingerprint,JSON_UNESCAPED_SLASHES),svsh_secret());
