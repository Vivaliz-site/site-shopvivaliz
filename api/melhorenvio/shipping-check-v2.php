...
$from=preg_replace('/\D+/','',svsh_env('MELHORENVIO_FROM_POSTAL_CODE','SHOPVIVALIZ_FROM_POSTAL_CODE'))?:'35500000';
...
$selected=$options[0]; $expiresAt=time()+1800; $fingerprint=['cep'=>$cep,'items'=>$fingerprintItems,'option'=>$selected,'expires_at'=>$expiresAt]; $secret=svsh_env('APP_KEY','SHOPVIVALIZ_APP_KEY','QUOTE_SIGNING_KEY')?:hash('sha256',__FILE__.php_uname()); $quoteId=hash_hmac('sha256',json_encode($fingerprint,JSON_UNESCAPED_SLASHES),$secret);
svsh_json(200,['ok'=>true,'provider'=>'melhorenvio','cep'=>$cep,'shipping_options'=>$options,'shipping_total'=>$selected['price'],'selected_option'=>$selected,'quote_id'=>$quoteId,'expires_at'=>$expiresAt]);
