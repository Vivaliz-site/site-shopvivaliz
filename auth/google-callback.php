...
function sv_google_oauth_signed_state_purpose(string $state): string
{
    ...
    return in_array($purpose, ['google_ads', 'gmail_readonly', 'gmail_returns'], true) ? $purpose : 'google_ads';
}
...
try {
    ...
    if (is_string($cloudProject) && $cloudProject !== '') {
        ...
        if (is_string($adsJob) && $adsJob !== '') {
            ...
            if ($googleOauthPurpose === 'gmail_readonly' || $googleOauthPurpose === 'gmail_returns') {
...
<?php elseif ($gmailOauthSuccess): ?><h1>Gmail autorizado</h1><p>A autorização de leitura e envio do Gmail foi concluída. Esta janela pode ser fechada.</p>
