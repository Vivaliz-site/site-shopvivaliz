...
- **`OLIST_ACCESS_TOKEN` (API v3) e' renovado com sucesso a cada 2h por `shopvivaliz-token-renewer.service`** (le refresh token, chama o endpoint OAuth, grava um novo access token — log mostra "Token Olist renovado" toda vez) **mas a API `GET /produtos` da Tiny retorna 401 mesmo com o token fresco.** Decodificando o JWT (base64 do segundo segmento, sem verificar assinatura): `roles.tiny-api` inclui `produtos-leitura`, token nao expirado, mas `scope: "email offline_access"` — sugere que o app OAuth usado (`azp` no JWT, um client_id `tiny-api-...`) pode nao ter o modulo "API de Produtos" habilitado nas permissoes do aplicativo (isso e' configurado por app, nao por usuario, segundo a doc https://ajuda.tiny.com.br/kb/articles/erp/integracoes/gestao-de-integracoes/aplicativos-api-v3-configuracoes-e-utilizacao). Nao encontrei essa tela de permissoes de app via navegacao no painel `erp.olist.com/integracoes` (isso parece ficar em outra area do painel, tipo "Configuracoes > Aplicativos") — **NAO E' o mesmo painel que controla webhooks/URLs de notificacao**, que fica em Integracoes > editar > Notificacoes.
- Servico `shopvivaliz-products-active-sync.service` criado (`scripts/products-active-sync-loop.sh`, loop com `sleep 10800` = 3h) para manter isso sincronizado automaticamente dali pra frente, mas **esta parado (`systemctl stop`) ate o problema de permissao do app OAuth ser resolvido** — reiniciar com `sudo systemctl start shopvivaliz-products-active-sync.service` assim que o token conseguir listar produtos (testar com `php /caminho/da/release/olist/sync-on-webhook.php` manualmente primeiro).
**Por quê importa:** o site nunca refletiu corretamente quais produtos estao ativos no ERP desde que o daemon foi escrito (nomes de coluna nunca bateram com o schema real) — nao e' uma regressao recente, e' um recurso que nunca funcionou. Qualquer agente que "descobrir" produtos inativos aparecendo como ativos deve ler esta entrada antes de reinvestigar do zero.
