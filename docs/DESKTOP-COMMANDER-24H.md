...
- Tarefa primaria: `AtStartup`, `LogonType S4U`, `RunLevel Highest`, sem depender de logon interativo.
...
- A tarefa habilita o log operacional `Microsoft-Windows-TaskScheduler/Operational` quando a política local permite.\n- Guardian independente: `ShopVivaliz DESKTOP-KOCEPSV Desktop Commander Task Guardian`, em `S4U`, `AtStartup` e a cada 15 minutos, recria/reabilita a tarefa principal se necessário.
...
## VMs Ubuntu
- Canal oficial Linux: Desktop Commander `0.2.51` pinado localmente e iniciado com `remote --persist-session`, sob usuário `ubuntu`.
...
- O instalador resolve Node/npx sob `ubuntu`, materializa o pacote `0.2.51` em caminho local pinado e grava os caminhos necessários em `/etc/default/shopvivaliz-desktop-commander`; o boot não depende de acesso ao registry.
...
- Nunca iniciar manualmente um Desktop Commander em host já gerenciado. Fred-Win permanece em `0.2.48`, DESKTOP-KOCEPSV usa `0.2.51` e as VMs Ubuntu usam `0.2.51` pinado localmente. Um processo manual reutiliza o mesmo Device ID e pode disputar presença com a sessão 24h.
