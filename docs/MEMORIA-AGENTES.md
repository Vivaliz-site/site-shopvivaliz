...
### 2026-07-18 — Pipeline "Otimização Inteligente Shopee" (CTR/título/A-B) já existe como GH Action e está quebrado silenciosamente — não reimplementar do zero
**Sistema/arquivo:** `.github/workflows/optimize-shopee-listings.yml` (cron diário `0 3 * * *`), `agents/v9.2.85/scripts/optimize-shopee-listings.php`, `agents/v9.2.85/app/ShopeeListingsOptimizationAgent.php`
**O que descobri:** existe uma task agendada (prompt "Agente de Otimização Inteligente Shopee", roda a cada 6h nesta sessão de Claude Code) que pede pra ler performance de vendas/CTR da API Shopee e otimizar títulos/descrições/imagens/preços. **Essa automação já existe de verdade** como GH Action (`optimize-shopee-listings.yml`, chama o agente PHP v9.2.85) com secrets reais (`TINY_ACCESS_TOKEN`, `ANTHROPIC_API_KEY` etc.) — uma sessão de Claude Code isolada **não tem** esses secrets (`env | grep -i shopee` vazio) nem acesso à API real de CTR/vendas da Shopee, então não há como executar o pipeline de verdade sem inventar dados (violaria a própria regra do prompt: "Análise deve ser baseada em dados, não suposições"). Além disso, descobri dois problemas reais na automação existente: 1) o último relatório real (`listings/optimization-report-20260717-093046.json`) tem `"status":"error"`, `"total_products": 0`, erro `"Falha OAuth2 refresh: Token is not active"` / `"Autenticação Tiny falhou (401)"` — mas o job do GitHub Actions termina com `exit 0` nesse caso (linha 76-78 do yml), então **a run aparece verde/"success" no Actions apesar de zero produtos otimizados** (falha silenciosa). 2) o cron `0 3 * * *` não disparou no dia 2026-07-18 (checado via `actions_list` às 08:21 UTC — último run real foi 2026-07-17T09:30:33Z, `total_count: 17`, nenhum run mais recente).
**Por quê importa:** se você é um agente futuro pegando essa mesma task agendada, **não tente reimplementar os 10-12 passos do prompt do zero** gerando/commitando dados de CTR ou "otimizações" fabricadas — isso duplicaria uma automação que já existe (exatamente o anti-padrão que este CLAUDE.md já alerta) e poluiria o repo com números inventados. O que falta de verdade é: (a) renovar `TINY_ACCESS_TOKEN` no GitHub Secrets (o refresh OAuth está inativo), e (b) investigar por que o schedule `0 3 * * *` parou de disparar (verificar Settings > Actions se o workflow não foi desabilitado, e se o repo teve algum push nos últimos 60 dias — teve, então não deveria ter sido auto-desabilitado por inatividade). Nenhuma das duas coisas é resolvível de dentro de uma sessão de Claude Code sem acesso a esses secrets/configurações — precisa de ação humana.
