...
### 2026-07-18/19 — MERCADOPAGO_WEBHOOK_SECRET REGREDIU 3 VEZES — causa raiz achada: workflow `sync-oracle-vm-secrets.yml` sobrescreve `.env` a cada 2h com secret do GitHub desatualizado [RESOLVIDO em 2026-07-19]
...
**O que descobri:** o bug já tinha "regredido pela 2a vez" (entrada anterior, sem causa raiz identificada). Na 3a ocorrência, confirmado no painel do Mercado Pago (`developers/panel/app/4737281715738852/webhooks`) que **0% das notificações estavam sendo entregues**, com HTTP 401 nos webhooks reais de HOJE (`payment.created` às 16:32 e 16:53 UTC). Rastreado até `.github/workflows/sync-oracle-vm-secrets.yml`: roda via cron **a cada 2 horas** (`5 */2 * * *`) e força `secrets.MERCADOPAGO_WEBHOOK_SECRET` (do GitHub Actions) para dentro do `.env` da VM via `scripts/update-production-env.py`, SEMPRE, sobrescrevendo qualquer correção manual feita direto no servidor. O secret do GitHub estava com o valor antigo/placeholder — cada vez que alguém corrigia o `.env` na mão (como nas 2 "correções" anteriores), o workflow revertia em até 2h.
**Por quê importa:** qualquer credencial que apareça nesse `ALLOWED_KEYS` de `update-production-env.py` (`MELHORENVIO_*`, `MERCADOPAGO_*`, `ML_*`, `OLIST_CLIENT_ID/SECRET`) tem esse mesmo risco — **corrigir só no `.env` do servidor NUNCA é suficiente pra essas chaves**, precisa corrigir também o secret correspondente no GitHub (`gh secret set NOME_DA_CHAVE`), senão o workflow desfaz a correção na próxima execução (até 2h depois). Corrigido em 2026-07-19: atualizado tanto `.env` da VM quanto o secret `MERCADOPAGO_WEBHOOK_SECRET` no GitHub com o valor real do painel MP (peguei via browser logado, `Configurar notificações` → `Modo de produção` → `Assinatura secreta`, ícone de olho pra revelar). Validado com "Simular notificação" no próprio painel MP: resposta `200 - OK`.
**Ver também:** entrada de token Olist abaixo (mesmo padrão: secret do GitHub desatualizado quebrando um workflow que sincroniza pra VM)
