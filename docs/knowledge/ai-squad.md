...
- OpenAI: `codex_chatgpt` com perfis ChatGPT Business autenticados → `chatgpt_browser` usando a sessão persistente do perfil `ai-squad-chatgpt-fallback` na VM backend → `manual_chatgpt` apenas se ambos os transportes automáticos falharem. O AI Squad não usa `OPENAI_API_KEY` como fallback.
...
Quando o fallback ChatGPT web recebe tarefa de programação/infra/API/segurança, ele deve pesquisar conhecimento atual na web antes de concluir, priorizando documentação oficial, repositórios oficiais, especificações, release notes, changelogs e issues, com URLs completas e verificação de versão/data.
