#!/usr/bin/env python3
"""
Extrair dados dos anuncios_*.xls usando xlrd
"""
import os
import openpyxl
...
print("\n" + "="*100)
print("EXTRAIR DADOS DOS ANUNCIOS (XLS) E GERAR PLANILHA FINAL")
print("="*100)
    ...
    try:
        ...
        for row in range(1, worksheet.nrows):
            ...
            for col_name in ["Produto", "SKU", "Sku", "PRODUCT_NUMBER"]:
                if col_name in headers_arquivo:
                    sku = worksheet.cell_value(row, headers_arquivo[col_name])
                    if sku:
                        break
...
if dados_anuncios:
    print(f"    Exemplo:")
    for id_tiny, dados in list(dados_anuncios.items())[:2]:
        print(f"      ID {id_tiny}: {dados}")
...
for row_orig in range(2, ws_orig.max_row + 1):
    ...
    # Procurar vermelhos (APENAS produtos que estao ABAIXO do ML)
...
# Salvar
output_path = Path(r"C:\Users\user\Downloads\PLANILHA_FINAL_IMPORTACAO.xlsx")
