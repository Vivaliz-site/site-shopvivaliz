#!/usr/bin/env python3
"""
Gerar planilha com APENAS os anuncios que vao sofrer alteracao
Se produto tem vermelho APENAS em Amazon, inclui so anuncio da Amazon
Se tem vermelho em Shopee e TikTok, inclui so esses dois
"""
import os
import openpyxl
...
print("\n" + "="*100)
print("GERAR PLANILHA - APENAS ANUNCIOS QUE SOFREM ALTERACAO")
print("="*100)
...
# Mapeamento: coluna -> (integracao, table_id)
MAPA_COL = {
    8: ("PDV", 982),
    9: ("Shopee", 989),
    10: ("Amazon", 991),
...
for row_orig in range(2, ws_orig.max_row + 1):
    ...
    # PASSO 1: Identificar QUAIS COLUNAS tem vermelho para este produto
    ...
    # PASSO 2: Para CADA COLUNA COM VERMELHO, procurar anuncio da integracao correspondente
    ...
    for col_idx in colunas_com_vermelho:
        integracao_esperada, table_id = MAPA_COL[col_idx]
        ...
        if anuncios_integracao:
            # Tem anuncio dessa integracao, incluir UMA LINHA POR ANUNCIO
            ...
            # Nao tem anuncio dessa integracao, incluir mesmo assim (sem ID de anuncio)
            row_count += 1
...
# Salvar
output_path = Path(r"C:\Users\user\Downloads\PLANILHA_ANUNCIOS_ALTERACAO.xlsx")
...
print(f"  (apenas anuncios que sofrem alteracao)")
print(f"\n  Atualizado com sucesso (OK): {total_ok}")
print(f"  Com atencao/erro/sem anuncio: {total_erro}")
