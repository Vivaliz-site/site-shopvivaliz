#!/usr/bin/env python3
"""
Gerar RELATORIO FINAL CORRETO:
- Apenas os 89 anuncios que foram processados
- 37 colunas (25 de ML vazias + 12 de Tiny preenchidas)
- Mesmo padrão do arquivo Anuncios-ML-com-precos-Tiny.xlsx
"""
import os
import requests
import openpyxl
...
print("\n" + "="*100)
print("GERAR RELATORIO FINAL - FORMATO CORRETO")
print("="*100)
...
# Headers - EXATAMENTE como no arquivo exemplo
headers_completos = [
    "FAMILY_ID",            # 1
    "ITEM_ID",              # 2
    "PRODUCT_NUMBER",       # 3
    ...
    "TITLE",                # 5
    "VARIATIONS",           # 6
    "STORE_STOCK_QUANTITY_58715512#BRP1129628561",  # 7
    "STOCK_FULL",           # 8
    "PRICE",                # 9
    "CURRENCY_ID",          # 10
    ...
    "SKU Tiny",                  # 27
    "Descrição Tiny",            # 28
    "Preço Mercado Livre original",  # 29
    "Preço cadastro Tiny",       # 30
    "Preço tabela PDV",          # 31
    "Preço tabela Shopee",       # 32
    "Preço tabela Amazon",       # 33
    "Preço tabela TikTok",       # 34
    "Status da localização",     # 35
    "Critério utilizado",        # 36
    "Observação"                 # 37
]

# Escrever headers
style_header = Font(bold=True, color="FFFFFF", size=9)
...
for row_orig in range(2, ws_orig.max_row + 1):
    ...
    # Colunas 1-25: VAZIAS (dados ML não preenchemos, conforme orientacao)
    # Deixar nulas propositalmente

    # Colunas 26-37: DADOS DO TINY
    ...
    if not tem_vermelho:
        ...
        ws_new.cell(linha_saida, 36).value = "Preco OK"
        ...
        if id_tiny not in produtos_tiny:
            ws_new.cell(linha_saida, 35).value = "ERRO"
            ws_new.cell(linha_saida, 36).value = "Nao encontrado"
            ws_new.cell(linha_saida, 37).value = "ID 404 no Tiny"
            total_erro += 1

            # Colorir linhas de erro em VERMELHO
            ...
            if abs(float(preco_atual) - preco_esperado) < 0.01:
                ws_new.cell(linha_saida, 35).value = "OK"
                ws_new.cell(linha_saida, 36).value = "Preco ML + 0.01"
                ws_new.cell(linha_saida, 37).value = f"R$ {preco_atual}"
                total_ok += 1

                # Colorir linhas OK em VERDE
                ...
                ws_new.cell(linha_saida, 35).value = "ATENCAO"
                ws_new.cell(linha_saida, 36).value = "Preco diferente"
                ws_new.cell(linha_saida, 37).value = f"Esperado R$ {preco_esperado}, tem R$ {preco_atual}"
                total_erro += 1

                # Colorir linhas com atencao em LARANJA
...
for col in range(1, 38):
    ...
    if col <= 25:
        ws_new.column_dimensions[col_letter].width = 12
    else:
...
# Salvar
output_path = Path(r"C:\Users\user\Downloads\RELATORIO_FINAL_ANUNCIOS_2026-07-26.xlsx")
...
print(f"\n  Total de anuncios: {total_processados}")
print(f"  OK: {total_ok}")
print(f"  Com atencao/erro: {total_erro}")
print(f"  Taxa de sucesso: {100*total_ok/total_processados:.1f}%")
