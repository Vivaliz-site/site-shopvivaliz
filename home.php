...
<head>
    ...
    <style>
        :root { --navy:#173B63; --green:#2DBB57; --green-soft:#DFF5E7; --ink:#102033; --muted:#5E7188; --line:#DBE5EF; }
        html, body { font-family:'Manrope','Segoe UI',sans-serif; background:radial-gradient(circle at top left, rgba(45,187,87,0.08), transparent 28%), linear-gradient(180deg, #F8FBFD 0%, #F3F7FB 100%); color:var(--ink); }
        .navbar { background:rgba(255,255,255,0.92); backdrop-filter:blur(16px); border-bottom:1px solid rgba(219,229,239,0.85); }
        .navbar .container { min-height:82px; }
        .hero-shell { padding:28px 0 28px; }
        .hero-grid { display:grid; grid-template-columns:minmax(0,1.35fr) minmax(280px,0.8fr); gap:20px; }
        .hero-card { border-radius:30px; padding:42px; background:radial-gradient(circle at top right, rgba(45,187,87,0.22), transparent 24%), linear-gradient(135deg, #143A62 0%, #20486F 44%, #2DBB57 125%); color:white; min-height:420px; display:flex; flex-direction:column; justify-content:space-between; box-shadow:0 28px 60px rgba(23,59,99,0.20); }
        ...
        .hero-btn-alt { background:rgba(255,255,255,0.10); color:white; border:1px solid rgba(255,255,255,0.16); }
        .hero-metrics { display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:12px; margin-top:28px; }
        .metric-box { background:rgba(255,255,255,0.10); border:1px solid rgba(255,255,255,0.12); border-radius:18px; padding:16px; }
        .metric-box strong { display:block; font-size:16px; margin-bottom:6px; }
        .metric-box span { color:rgba(255,255,255,0.80); font-size:13px; }
        .hero-side { display:grid; gap:16px; }
        .mini-banner, .category-card, .product-card { background:white; border:1px solid var(--line); border-radius:24px; box-shadow:0 18px 40px rgba(15,23,42,0.06); }
        .mini-banner { padding:24px; }
        .mini-banner strong { color:var(--navy); font-size:13px; text-transform:uppercase; letter-spacing:0.08em; }
        .mini-banner h2 { margin:12px 0 10px; font-size:24px; }
        .mini-banner p, .section-head p, .category-card p { color:var(--muted); line-height:1.7; }
        .mini-banner a, .section-head a { color:var(--navy); font-weight:800; text-decoration:none; }
        .value-strip { margin:28px 0 34px; padding:24px 28px; border-radius:26px; background:linear-gradient(135deg, #0F2843 0%, #173B63 45%, #204C77 100%); color:white; display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:18px; }
        .value-strip strong { display:block; font-size:19px; margin-bottom:8px; }
        .value-strip p { color:rgba(255,255,255,0.80); line-height:1.7; font-size:14px; }
        .section-shell { padding:22px 0 8px; }
        .section-head { display:flex; justify-content:space-between; align-items:end; gap:16px; margin-bottom:18px; }
        .section-head h2 { font-size:30px; margin-bottom:6px; }
        .categories-grid, .products-grid, .footer-grid { display:grid; gap:16px; }
        .categories-grid { grid-template-columns:repeat(4, minmax(0, 1fr)); }
        .category-card { padding:24px; text-decoration:none; color:inherit; }
        .category-pill { display:inline-flex; align-items:center; justify-content:center; min-width:42px; height:42px; border-radius:12px; background:var(--green-soft); color:var(--green); font-weight:800; margin-bottom:16px; }
        .products-grid { grid-template-columns:repeat(4, minmax(0, 1fr)); }
        .product-image { background:#F7FBFD; height:240px; display:flex; align-items:center; justify-content:center; overflow:hidden; border-radius:24px 24px 0 0; }
        .product-image img { width:100%; height:100%; object-fit:cover; }
        .product-body { padding:18px; }
        .product-tag-row { display:flex; flex-wrap:wrap; gap:8px; margin-bottom:12px; }
        .product-tag { padding:6px 10px; border-radius:999px; background:#EEF5FB; color:var(--navy); font-size:11px; font-weight:800; text-transform:uppercase; }
        .product-title { font-size:16px; line-height:1.45; min-height:46px; margin-bottom:14px; font-weight:700; }
        .product-meta { display:flex; justify-content:space-between; align-items:center; gap:12px; }
        .product-meta span { font-size:12px; color:var(--muted); }
        .product-link { min-width:108px; background:var(--green); color:white; border-radius:14px; padding:11px 14px; }
        .empty-products { background:white; border:1px dashed var(--line); border-radius:22px; padding:30px; color:var(--muted); text-align:center; }
        footer { margin-top:46px; background:#0F2843; color:white; padding:34px 0; }
        .footer-grid { grid-template-columns:1.2fr repeat(2, minmax(0, 1fr)); }
        .footer-grid p, .footer-grid a { color:rgba(255,255,255,0.78); text-decoration:none; line-height:1.7; font-size:14px; }
        .footer-grid h3 { margin-bottom:10px; font-size:15px; }
