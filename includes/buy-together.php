 ...
 *
 * - 3% de desconto sobre os dois SKUs que formam o par.
 * - O navegador apenas informa quais SKUs compoem a oferta.
 * - Preco, quantidade e valor do desconto sao sempre recalculados a partir
 *   dos itens autoritativos ja validados pelo servidor.
 * - O beneficio pode coexistir com um cupom; o cupom e calculado depois do
 *   desconto do compre-junto para evitar desconto sobre valor que ja foi
 *   abatido.
 */
...
function svbt_validate_offer(mixed $rawOffer, array $authoritativeItems): array
{
    ...
    if ($offer === []) {
        return svbt_empty_result();
    }

    $type = strtolower(trim((string)($offer['type'] ?? '')));
    if ($type !== SVBT_OFFER_TYPE) {
        return svbt_empty_result('invalid_offer_type');
    }
    ...
    if (count($skus) !== 2) {
        return svbt_empty_result('offer_requires_two_distinct_skus');
    }
    ...
    foreach ($authoritativeItems as $item) {
        ...
        $itemsBySku[strtolower($sku)] = $item;
    }
    ...
    foreach ($skus as $key => $originalSku) {
        ...
        if (!is_array($item)) {
            return svbt_empty_result('offer_item_missing');
        }

        $price = round(max(0.0, (float)($item['price'] ?? 0)), 2);
        $quantity = max(0, (int)($item['quantity'] ?? 0));
        if ($price <= 0 || $quantity <= 0) {
            return svbt_empty_result('offer_item_invalid');
        }
    ...
    $amount = round($eligibleSubtotal * SVBT_DISCOUNT_PERCENT / 100, 2);
    if ($amount <= 0) {
