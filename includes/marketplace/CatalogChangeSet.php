 ...
 *
 * Este modulo nao publica, nao altera preco/estoque e nao consulta rede.
 * Ele apenas compara o snapshot atual do canal com o conteudo aprovado no
 * staging, respeitando o field_map canonico de CatalogChannelProfile.php.
 */
 ...
 * @param array<string,mixed> $after Conteudo do staging/formulario.
 * @param array<string,array<string,mixed>> $fieldMap Mapa canonico do canal.
 * @return array<string,mixed>
 */
function sv_catalog_change_set(array $before, array $after, array $fieldMap): array
{
    ...
    foreach (sv_catalog_change_field_keys() as $field => $contentKey) {
        ...
        $all[] = $row;
        if ($mode === 'internal') {
            if ($changed) $internal[] = $row;
            continue;
        }
        if ($changed) $effective[] = $row;
        else $unchangedEffective[] = $row;
    }
