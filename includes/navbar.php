...
<nav class="navbar sv-navbar"><div class="container nav-inner"><a href="/" class="brand-link" aria-label="Ir para a home da Vivaliz"><img src="/images/logo-vivaliz.png" alt="Vivaliz" class="brand-logo-img" width="210" height="46" decoding="async" onerror="this.src='/images/logo-vivaliz-square.png'"></a><button class="menu-toggle" id="menuToggle" type="button" aria-expanded="false" aria-controls="navMenu" aria-label="Abrir menu"><span aria-hidden="true">☰</span></button><div class="navbar-menu" id="navMenu"><?php foreach ($svNavLinks as $link): ?><?php $isCurrent = in_array($svNavCurrent, $link['match'], true); ?><a href="<?= htmlspecialchars($link['href'], ENT_QUOTES, 'UTF-8') ?>"<?= $isCurrent ? ' aria-current="page"' : '' ?><?= $link['href'] === '/catalogo' ? ' class="sv-nav-cta"' : '' ?>><?= htmlspecialchars($link['label'], ENT_QUOTES, 'UTF-8') ?><?php if ($link['href'] === '/carrinho'): ?><span class="cart-badge" id="nav-cart-count" aria-live="polite"></span><?php endif; ?></a><?php endforeach; ?><?php if ($svLoggedIn): ?><a href="/meus-pedidos.php" class="nav-account-link">👤 <?= htmlspecialchars($svUserFirstName, ENT_QUOTES, 'UTF-8') ?></a><a href="/auth/logout.php">Sair</a><?php else: ?><a href="/auth/login.php" class="nav-account-link">Entrar</a><?php endif; ?></div></div></nav>
<div class="sv-liz-assistant" data-endpoint="/api/agent/squad-chat.php"><button class="sv-liz-trigger" type="button" aria-expanded="false" aria-controls="svLizPanel"><img src="/images/logo-vivaliz-square.png" alt="Assistente Liz" width="48" height="48" decoding="async"><span class="sv-liz-trigger-copy"><strong>Fale com a Liz</strong><small>Ajuda para encontrar produtos</small></span></button><section class="sv-liz-panel" id="svLizPanel" hidden aria-label="Assistente virtual Liz"><header><img src="/images/logo-vivaliz-square.png" alt="" width="42" height="42"><div><strong>Liz</strong><span>Assistente virtual ShopVivaliz</span></div><button class="sv-liz-close" type="button" aria-label="Fechar">×</button></header><div class="sv-liz-messages" aria-live="polite"><div class="sv-liz-message is-liz">Olá! Posso ajudar você a encontrar um produto, comparar opções ou tirar dúvidas sobre a loja.</div></div><form class="sv-liz-form"><label class="sr-only" for="svLizInput">Digite sua mensagem</label><input id="svLizInput" type="text" maxlength="500" placeholder="Como posso ajudar?" autocomplete="off"><button type="submit">Enviar</button></form></section></div>
...
<?php if ($svIsCheckout): ?><script src="/js/checkout-conversion-v6.js?v=6.0.0" defer></script><script src="/js/checkout-resilience-v15.js?v=15.0.0" defer></script><script src="/js/checkout-shipping-v7.js?v=7.0.0" defer></script><script src="/js/checkout-cart-freshness-v93.js?v=93.0.0" defer></script><?php endif; ?>
