...
final class SvFallbackPdo extends PDO
{
    ...
    private function saveSettings(): void
    {
        @file_put_contents(
            $this->settingsFile,
            json_encode($this->settings, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
        );
    }

    private function paramValue(array $params, int|string $key, mixed $default = null): mixed
    {
        if (array_key_exists($key, $params)) {
            return $params[$key];
        }

        $cleanKey = is_string($key) ? ltrim($key, ':') : (string)$key;
        if ($cleanKey !== '' && array_key_exists(':' . $cleanKey, $params)) {
    ...
    {
        $sql = strtolower(trim((string)$statement));
        if ($sql === '') {
            return 0;
        }

        if (str_contains($sql, 'insert into site_settings') || str_contains($sql, 'update site_settings')) {
            return 1;
        }
    ...
    {
        return $attribute === PDO::ATTR_DRIVER_NAME ? 'fallback' : null;
    }
    ...
    {
        $normalized = strtolower(preg_replace('/\s+/', ' ', trim($sql)) ?: trim($sql));
        $rowCount = 0;

        if (str_contains($normalized, 'from site_settings') && str_starts_with($normalized, 'select')) {
            $key = (string)$this->paramValue($params, ':k', $this->paramValue($params, 'k', ''));
            if ($key !== '') {
            ...
            foreach ($this->settings as $settingKey => $settingValue) {
                $rows[] = ['setting_key' => $settingKey, 'setting_value' => $settingValue];
            }
            $rowCount = count($rows);
            return $rows;
        }
        ...
        if (str_contains($normalized, 'insert into site_settings')) {
            $key = (string)$this->paramValue($params, ':k', $this->paramValue($params, 'k', ''));
            $value = (string)$this->paramValue($params, ':v', $this->paramValue($params, 'v', ''));
            if ($key !== '') {
