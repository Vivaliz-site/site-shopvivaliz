(function(){
  ...
  window.svCheckoutErrorMessage=function(code,fallback){return messages[code]||fallback||'Não foi possível concluir o pedido agora.';};
  form.addEventListener('submit',function(){
    var button=document.getElementById('submit-btn');
    if(button){button.dataset.originalText=button.dataset.originalText||button.textContent;setTimeout(function(){if(button.disabled&&button.textContent==='Enviando…')button.textContent='Processando com segurança…';},2500);}
    if(status){status.setAttribute('role','status');status.setAttribute('aria-live','polite');}
  },true);
  window.addEventListener('offline',function(){if(status){status.textContent='Você está sem conexão. O pedido não foi enviado.';status.className='checkout-status-msg err';}});
})();
