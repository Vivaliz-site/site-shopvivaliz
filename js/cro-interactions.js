...
(function () {
    ...
    function initSkeletonLoaders() {
        ...
        images.forEach((img) => {
            ...
            if (img.complete) {
                parent.classList.remove('skeleton', 'product-image-skeleton');
            } else {
     ...
     *
     * O limite é sempre lido do endpoint oficial de configurações. Nada neste
     * arquivo inventa valor mínimo, cupom ou benefício adicional.
     */
    function initFreeShippingProgress() {
        ...
        const wrappers = Array.from(bars).map((bar) => (
            bar.closest('.free-shipping-progress-wrapper, .free-shipping-rewards, .gamification-rewards-container') || bar
        ));

        texts.forEach((text) => { text.textContent = ''; });
        ...
        fetch('/api/settings/free-shipping.php', { cache: 'no-store' })
            .then((response) => {
                if (!response.ok) throw new Error('free_shipping_config_http_' + response.status);
                return response.json();
            })
            .then((config) => {
                const limit = Number(config && config.threshold);
                if (!config || !config.enabled || !Number.isFinite(limit) || limit <= 0) {
                    return;
                }
                ...
                window.updateFreeShippingVisual = function () {
                    const totalString = cartTotalEl.innerText
                        .replace('R$', '')
                        .replace(/\./g, '')
                        .replace(',', '.')
                        .trim();
                    const currentTotal = Math.max(0, parseFloat(totalString) || 0);
                    const percentage = Math.min(100, (currentTotal / limit) * 100);
                    ...
                    texts.forEach((text) => {
                        ...
                        const remaining = (limit - currentTotal).toFixed(2).replace('.', ',');
            ...
            .catch(() => {
                // Falha fechada: se a configuração não puder ser confirmada,
                // o benefício permanece oculto em vez de ser prometido.
            });
    }
