...
# Auditor implementations are validated by unit tests and the changed-file
# governance gate. Excluding them here prevents their detection regexes and
# controlled subprocess probes from being mistaken for operational behavior.
EXCLUDED_FILES = {
...
PATH_REFERENCE = re.compile(
    r"(?<![A-Za-z0-9_.-])((?:scripts|agent-bridge|ai-system|agents|api/agent|admin|"
    r"includes|deploy/systemd)/[A-Za-z0-9_./-]+(?:\.(?:py|js|ts|php|sh|bash|ps1|"
    r"service|timer))?)"
)

EVIDENCE = re.compile(
    r"(?:evidence|commit_sha|pull_request|pr_url|artifact|test_(?:report|result)|"
    r"tests_passed|verification|read[-_ ]?back|run_id)", re.I,
)
COMPLETION = re.compile(
    r"(?:status|state)\s*[:=]\s*[\"'](?:completed|success)[\"']|"
    r"[\"']status[\"']\s*:\s*[\"']ok[\"']|"
    r"[\"'](?:success|ok)[\"']\s*:\s*(?:true|True)", re.I,
)
SIMULATED_SUCCESS = re.compile(
    r"[\"']simulated[\"']\s*:\s*(?:true|True)|"
    r"(?:status|state)[\"']?\s*[:=]\s*[\"'](?:simulated|mock|fake)[\"']|"
    r"\b(?:fake|mock)\s+(?:success|execution|result)\b", re.I,
)
QUEUE_MUTATION = re.compile(
    r"\[\s*[\"']status[\"']\s*\]\s*=\s*[\"'](?:in_progress|processing|"
    r"completed|done|success)[\"']|"
    r"\[\s*[\"']assigned_to[\"']\s*\]\s*=", re.I,
)
PRINTED_EXECUTION = re.compile(
    r"(?:Executando comando|Executing command|comando de teste)", re.I,
)
EXECUTION_CALL = re.compile(
    r"(?:subprocess\.(?:run|Popen|check_output)|run_shell\(|exec\(|shell_exec\(|"
    r"proc_open\()", re.I,
)
MASKED_FAILURE = re.compile(
    r"(?m)(?:\|\|\s*(?:true|log\b|echo\b)|continue-on-error\s*:\s*true|"
    r"^\s*set\s+\+e\s*$)", re.I,
)
CHECK_FALSE = re.compile(r"check\s*=\s*False", re.I)
RETURN_CODE_CHECK = re.compile(
    r"(?:returncode|return_code|exit_code|\$LASTEXITCODE)", re.I,
)
...
DANGEROUS: tuple[tuple[str, str, re.Pattern[str], str], ...] = (
    ...
    (
        ...
        "Automation must not merge itself.",
    ),
    (
        "destructive_reset", "critical",
        re.compile(
            r"(?m)^\s*(?:git\s+reset\s+--hard\b|git\s+clean\s+-[a-z]*f[a-z]*d|"
            r"rm\s+-rf\s+/\s*$)", re.I,
        ),
...
WEB_MUTATION_MARKERS = (
    "raw.githubusercontent.com", "file_put_contents(", "git fetch",
    "git merge --ff-only", "opcache_reset", "shell_exec(", "exec(",
)
...
REMOTE_EXEC = (
    "shell=true", "subprocess.check_output(", "subprocess.run(",
    "shell_exec(", "exec(",
)
AUTH_MARKERS = (
    "authorization", "bearer ", "x-api-key", "hmac", "compare_digest",
...
def candidate(rel: str, path: Path) -> bool:
    ...
    if (
        ...
        or rel in ROOT_AUTOMATION_FILES
    ):
        return True
    if path.suffix:
        return False
...
def workflow_active(text: str) -> bool:
    return bool(
        re.search(r"(?m)^on:\s*(?:$|\{)", text)
        and ACTIVE_EVENT.search(text)
    )
...
def active_surfaces(files: list[Path]) -> set[str]:
    ...
    for path in files:
        ...
        if rel.startswith(".github/workflows/") and workflow_active(text):
            ...
            rel.startswith("deploy/systemd/")
            and path.suffix.lower() in {".service", ".timer"}
        ):
            active.add(rel)
            texts.append(text)
...
def fingerprint(rule: str, rel: str, excerpt: str) -> str:
...
def add_finding(
    ...
    findings.append(
        Finding(
            ...
            active=active,
            fingerprint=fingerprint(rule, rel, value),
        )
    )
...
def audit_text(rel: str, original: str, active: bool) -> list[Finding]:
    if rel in EXCLUDED_FILES or not automation_path(rel):
        return []
    ...
    evidence_sensitive = any(
        token in rel.lower()
        for token in (
            "agent", "task", "audit", "guardian", "orchestrator", "bridge",
            "worker",
        )
    )
    ...
    if simulation and completion:
        add_finding(
            findings, "critical", "simulated_completion", rel, text,
            simulation, active,
            "Simulated state and successful completion coexist.",
        )
    if completion and not evidence and evidence_sensitive:
        add_finding(
            findings, "high", "completion_without_evidence", rel, text,
            completion, active,
            "Successful completion lacks commit, test, verification or artifact evidence.",
        )

    for rule, severity, pattern, message in DANGEROUS:
        for match in pattern.finditer(text):
            add_finding(
                findings, severity, rule, rel, text, match, active, message,
            )

    queue = QUEUE_MUTATION.search(text)
    if queue and not evidence:
        add_finding(
            findings, "critical", "queue_mutation_without_evidence", rel,
            text, queue, active,
    ...
    if (
        masked
        and evidence_sensitive
        and (
            completion
            or re.search(
                r"cycle finished|runtime operacional|setup completo", text, re.I,
            )
        )
    ):
        add_finding(
            findings, "high", "masked_failure_with_success", rel, text,
            masked, active,
            "Failure is ignored while the routine can report success.",
        )

    unchecked = CHECK_FALSE.search(text)
    if unchecked and not RETURN_CODE_CHECK.search(text) and evidence_sensitive:
        add_finding(
            findings, "high", "unchecked_process_returncode", rel, text,
            unchecked, active,
            "A subprocess disables automatic checking without inspecting its return code.",
        )

    printed = PRINTED_EXECUTION.search(text)
    if printed and not EXECUTION_CALL.search(text):
        add_finding(
            findings, "high", "printed_execution_without_execution", rel,
            text, printed, active,
            "A command is logged as executing without a command runner or API call.",
        )

    unconditional = UNCONDITIONAL_RETURN_ZERO.search(text)
    if (
        unconditional
        and any(token in rel.lower() for token in ("guardian", "orchestrator"))
        and RETURN_CODE_CHECK.search(text)
    ):
        add_finding(
            findings, "high", "unconditional_success_after_failure", rel,
            text, unconditional, active,
            "Failure-capable automation returns success unconditionally.",
        )

    if rel.startswith(("admin/", "api/")):
        marker_count = sum(marker in lowered for marker in WEB_MUTATION_MARKERS)
        mutation = (
            "file_put_contents(" in lowered
            and "raw.githubusercontent.com" in lowered
        ) or (
        ...
        if mutation and marker_count >= 2:
            add_finding(
                findings, "critical", "web_triggered_code_mutation", rel,
                text, None, active,
                "A web endpoint can download, overwrite or fast-forward executable production code.",
            )

    permission = UNSAFE_PRIVATE_PERMISSION.search(text)
    if permission and PRIVATE_WORD.search(text):
        add_finding(
            findings, "critical", "unsafe_private_directory_permission", rel,
            text, permission, active,
            "Broad write access is granted on private or credential state.",
        )

    is_server = (
        "agent" in rel.lower()
        and any(marker in lowered for marker in REMOTE_EXEC)
    )
    if (
        is_server
        and "0.0.0.0" in lowered
        and not any(marker in lowered for marker in AUTH_MARKERS)
    ):
        add_finding(
            findings, "critical", "unauthenticated_remote_execution", rel,
            text, None, active,
            "Command-capable agent server binds publicly without authentication.",
        )

    if rel.startswith("deploy/systemd/") and "execstart=" in lowered:
        retired = (
            "autonomous-continuous-cycle.py",
            "agent-operations-worker.py",
            "scripts/autonomous-executor.py",
        )
        marker = next((item for item in retired if item in lowered), None)
        if marker:
            add_finding(
                findings, "critical", "systemd_runs_retired_executor", rel,
                text, None, True,
                "A systemd unit runs a retired or non-evidence-based executor.",
                marker,
            )
...
def baseline_fingerprints(
    files: list[Path], active: set[str], baseline: str | None,
) -> set[str]:
    if not baseline:
        return set()
    values: set[str] = set()
    for path in files:
        ...
        if result.returncode == 0:
            ...
            values.update(
                item.fingerprint
                for item in audit_text(rel, old_text, rel in active)
            )
    return values
...
def main() -> int:
    ...
    findings.sort(
        key=lambda item: (
            order.get(item.severity, 9), item.path, item.line or 0, item.rule,
        )
    )
    blocking = [
    ...
    payload = {
        ...
        "legacy_debt": sum(
            item.preexisting and not item.active for item in findings
        ),
        "fail_closed": True,
        "coverage": {
            "workflows": True,
    ...
    lines = [
        ...
        f"- Base: `{baseline or 'current-state'}`",
        f"- Files scanned: **{len(files)}**",
        f"- Hooks scanned: **{len(hooks)}**",
        f"- systemd units scanned: **{len(units)}**",
