...
validation = {
    ...
    "response_sha256": __import__("hashlib").sha256(Path(response_path).read_bytes()).hexdigest(),
    "persisted_sha256": __import__("hashlib").sha256(Path(persisted_path).read_bytes()).hexdigest(),
}
...
run_cycle() {
  ...
  require_file "$PROJECT_DIR/storage/agent-evidence/latest-agent-cycle.json"
  ...
  if ! validate_cycle \
      "$tmp_response" \
      "$PROJECT_DIR/storage/agent-evidence/latest-agent-cycle.json" \
      "$release_sha"; then
