...
$providedKey = trim((string)($_SERVER['HTTP_X_AUDIT_KEY'] ?? ''));
if ($providedKey === '' || !hash_equals('__AUDIT_TOKEN_HASH__', hash('sha256', $providedKey))) {
...
$expectedSha = 'cdcd9b4a9d66e860decb4ee36729d25e97dae385';
$path = svmp_find_order_path($orderNumber);
if ($path === '') {
    http_response_code(500);
    echo json_encode(['ok' => false, 'error' => 'order_file_missing']);
    exit;
}
...
foreach ($rows as $row) {
    ...
    $candidateTinyId = trim((string)($row['olist_order_id'] ?? ''));
    if ($candidateTinyId !== '') {
        $dbTinyIds[$candidateTinyId] = true;
    }
}
...
http_response_code($failures === [] ? 200 : 500);
echo json_encode([
    'ok' => $failures === [],
    'read_only' => true,
