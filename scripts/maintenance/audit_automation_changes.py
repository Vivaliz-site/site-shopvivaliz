...
AUDIT_WORDS = ("audit", "health", "incident", "agent", "guardian", "orchestrator", "bridge", "worker")
EVIDENCE_WORDS = (
    "artifact", "evidence", "commit_sha", "pull_request", "pr_url", "test_report",
    "run_id", "read-back", "readback", "verification",
)

LINE_RULES: tuple[tuple[str, str, re.Pattern[str], str], ...] = (
    (
        "critical", "auto_merge",
        re.compile(r"(?:gh\s+pr\s+merge\b[^\n]*--auto|enable_auto_merge\s*\(|auto-merge\s*:\s*true)", re.I),
        "Auto-merge was introduced.",
    ),
    (
        "critical", "destructive_command",
        re.compile(r"(?:git\s+reset\s+--hard\b|git\s+clean\s+-[a-z]*f[a-z]*d|git\s+checkout\s+--\s+\.|rm\s+-rf\s+/(?:\s|$))", re.I),
    ...
    (
        ...
        re.compile(r"git\s+push\b[^\n]*(?:--force(?:-with-lease)?|(?:HEAD:|refs/heads/)?(?:main|master)\b)", re.I),
        "Protected-branch or force push was introduced.",
    ),
    (
        "high", "fail_open",
        re.compile(r"(?:^\s*set\s+\+e\s*$|continue-on-error\s*:\s*true|^\s*exit\s+0\s*$)", re.I),
        "Fail-open behavior can mark a failed operation green.",
    ),
    (
        "high", "ignored_failure",
        re.compile(r"(?:\|\|\s*(?:true|log\b|echo\b)|if-no-files-found\s*:\s*(?:warn|ignore))", re.I),
...
COMPLETION_PATTERN = re.compile(
    r"(?:status|state)[^\n:=]{0,32}[:=]\s*['\"]?(?:completed|concluido|concluído|success|ok)"
    r"|completed_with_evidence|success\s*:\s*true|ok\s*:\s*true",
    re.I,
)
QUEUE_PATTERN = re.compile(r"(?:task|queue|fila|last_result|completed_at|assigned_to|in_progress)", re.I)
...
SIMULATION_PATTERN = re.compile(
    r"(?:['\"]simulated['\"]\s*:\s*true|status\s*[:=]\s*['\"](?:simulated|mock|fake)['\"]|"
    r"fake\s+(?:success|execution|result)|mock\s+(?:success|execution|result))",
    re.I,
)
PRINTED_EXECUTION_PATTERN = re.compile(r"(?:Executando comando|Executing command|comando de teste)", re.I)
EXECUTION_CALL_PATTERN = re.compile(r"(?:subprocess\.(?:run|Popen|check_output)|run_shell\(|exec\(|shell_exec\(|proc_open\()", re.I)
WEB_MUTATION_PATTERN = re.compile(
    r"raw\.githubusercontent\.com[^\n]*(?:/main/|/master/)|"
    r"(?:git\s+fetch|git\s+merge\s+--ff-only)[\s\S]{0,500}(?:exec\(|shell_exec\()",
    re.I,
)
UNSAFE_PRIVATE_PERMISSION_PATTERN = re.compile(
    ...
    r"(?:private|token|secret|credential|\.env)[^\n]*(?:chmod\s+0?77[07]|\b-m\s+2?770\b|\b-m\s+0?777\b)",
    re.I,
)
...
def scan_file(base: str, head: str, status: str, path: str) -> list[Finding]:
    ...
    if status == "D" and path in CRITICAL_WORKFLOWS:
        return [Finding("critical", "critical_workflow_removed", path, None, "A required audit/governance workflow was removed.", "deleted")]
    ...
    for line_no, text in additions:
        if any(pattern.search(text) for pattern in SECRET_PATTERNS):
            findings.append(Finding(
                ...
                "A credential-like value was introduced in tracked text.", redact(text),
            ))

    # Policy prose remains subject to secret scanning but is not executable.
    ...
    if simulation and completion:
        findings.append(Finding(
            "critical", "simulated_completion", path, line_for_match(additions, simulation),
    ...
    if completion and queue_change and not evidence:
        findings.append(Finding(
            "high", "queue_completion_without_evidence", path, line_for_match(additions, completion),
    ...
    if queue_mutation and not evidence:
        findings.append(Finding(
            "critical", "queue_mutation_without_evidence", path, line_for_match(additions, queue_mutation),
    ...
    if printed and not EXECUTION_CALL_PATTERN.search(added_text):
        findings.append(Finding(
            "high", "printed_execution_without_execution", path, line_for_match(additions, printed),
    ...
    if web_mutation or (
        ...
        match = web_mutation or re.search(r"raw\.githubusercontent\.com", added_text, re.I)
        findings.append(Finding(
    ...
    if lower_path.startswith(".github/workflows/"):
        ...
        mutation = re.search(
            r"(?:git\s+push|gh\s+pr\s+merge|gh\s+issue\s+(?:edit|close)|"
