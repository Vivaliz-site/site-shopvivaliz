#!/usr/bin/env python3
"""Otimiza catalogo Shopee sem alterar preco ou estoque.

Este modulo e a implementacao canonica. O arquivo legado
`scripts/shopee_full_catalog_optimizer.py` deve permanecer apenas como wrapper
compatibilidade durante a migracao estrutural.
"""
...
BACKUP_DIR = PROJECT_ROOT / "storage" / "private" / "shopee-optimizer-backups"
REPORT_DIR = PROJECT_ROOT / "logs" / "shopee-optimizer"
...
def process_images(client: ShopeeClient, item: dict[str, Any]) -> tuple[list[str], dict[str, Any]]:
    """Generate, validate and upload images. Imported lazily to keep SEO-only runs light."""
    from ia.images.image_generator import generate_for_product  # noqa: WPS433
    from ia.images.image_validator import validate_batch  # noqa: WPS433
...
def main() -> int:
    ...
    for index, item in enumerate(details, start=1):
        ...
        if apply and not validation_errors:
            apply_product_update(client, before, title=title, description=description, images=images)
            entry["updated"] = True
