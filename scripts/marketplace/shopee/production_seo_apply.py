...
Canonical implementation lives in scripts/marketplace/shopee. Legacy wrappers in
scripts/ keep backwards compatibility during repository cleanup.
"""
...
from full_catalog_optimizer import build_description, build_title, image_ids, smart_validate  # noqa: E402
from utils.shopee_client import ShopeeClient  # noqa: E402

CONFIRMATION = "APPLY_ALL_SHOPEE_PRODUCTS"
BACKUP_DIR = PROJECT_ROOT / "storage" / "private" / "shopee-production-backups"
REPORT_DIR = PROJECT_ROOT / "logs" / "shopee-production-seo"
...
def apply_one(client: ShopeeClient, before: dict[str, Any]) -> dict[str, Any]:
    ...
    if validation_errors:
        return {"item_id": item_id, "status": "blocked", "evidence": {"validation_errors": validation_errors}}
    ...
    try:
        ...
        raise RuntimeError(f"item={item_id} update_failed={update_error}; rollback={rollback}") from update_error
...
def main() -> int:
    ...
    if len(details) != len(item_ids):
        print(f"ERROR: expected {len(item_ids)} product details but received {len(details)}", file=sys.stderr)
        return 3
    ...
    for index, before in enumerate(details, start=1):
        try:
            ...
            fatal_failures += 1
            entry = {"item_id": before.get("item_id"), "status": "failed", "evidence": {"error": str(exc)}}
