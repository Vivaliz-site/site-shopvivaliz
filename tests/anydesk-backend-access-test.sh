...
grep -q 'set_password_ephemeral' "$setup"
grep -q 'SECRET_BRIDGE_DIR=' "$setup"
grep -q 'openssl pkeyutl -decrypt' "$setup"
grep -q 'rsa_padding_mode:oaep' "$setup"
grep -q 'rsa_oaep_md:sha256' "$setup"
grep -q 'anydesk --set-password' "$setup"
grep -q 'admin_security' "$setup"
grep -q 'anydesk --admin-settings:security' "$setup"
grep -q 'anydesk_set_password_ephemeral' "$oci_workflow"
