...
$assert(str_contains($timer, 'OnCalendar=*:0/30'), 'reconcile timer must use a calendar schedule every thirty minutes');
$assert(!str_contains($timer, 'OnUnitInactiveSec='), 'reconcile timer must not depend on unit inactive transitions');
