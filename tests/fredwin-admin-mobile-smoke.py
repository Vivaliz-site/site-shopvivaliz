...
def goto(p,u,ms=2300):
 r=p.goto(B+u,wait_until='domcontentloaded',timeout=35000);p.wait_for_timeout(ms);return r.status if r else 0
def login(w,ch):
 ...
 try:
  s=goto(p,'/auth/login.php',900);x=p.evaluate("""()=>{let n=[...document.querySelectorAll('a,button')].find(x=>/entrar com google/i.test(x.textContent||'')),r=n?.getBoundingClientRect();return{v:!!(n&&r&&r.width>1&&r.height>1),h:n?.getAttribute?.('href')||''}}""");return {'status':s,'google_visible':x['v'],'google_canonical':path(x['h'])=='/auth/google-start.php'}
 finally:c.close();b.close()
def home(p):
 s=goto(p,'/admin/',2500);a=auth(p)
 if not a['authenticated']:return {'authenticated':False,'path':a['path'],'status':s}
 ...
 if x['details']:
  ...
  if k:
   p.wait_for_timeout(250);p.reload(wait_until='domcontentloaded',timeout=35000)
 ...
 return {'authenticated':True,'status':s,'details':x['details'],'opened':op,'closed':cl,'saved':sv,'actions':{'Abrir seções','Recolher seções'}<=set(x['actions']),'dock':x['dock'] and e<=set(x['paths']),'padding':x['padding'],'overflow':x['overflow']}
def catalog(p):
 s=goto(p,'/admin/catalog-optimization/admin_catalog.php',2400);a=auth(p)
 if not a['authenticated']:return {'authenticated':False,'path':a['path'],'status':s}
 ...
 return {'authenticated':True,'status':s,'sort':v,'options':{'recent','urgent','channel','status','product'}<=set(o),'product':pr==['Alpha','Beta','Zeta'],'urgent':ur==['Zeta','Beta','Alpha'],'overflow':p.evaluate('()=>Math.max(0,document.documentElement.scrollWidth-innerWidth)')}
def image(p):
 s=goto(p,'/admin/ai-image-studio/admin_dashboard.php',3000);a=auth(p)
 if not a['authenticated']:return {'authenticated':False,'path':a['path'],'status':s}
 ...
 if z:
  ...
  p.reload(wait_until='domcontentloaded',timeout=35000);p.wait_for_timeout(2700);re=p.evaluate("x=>{let i=document.querySelector('.iv-item[data-product-id=\"'+CSS.escape(x.id)+'\"]');if(!i)return false;let p=i.querySelector(':scope>summary input.iv-check'),v=[...i.querySelectorAll('.iv-variant input.iv-check')].find(n=>n.value===x.type);return !!(p?.checked&&v?.checked)}",z)
 return {'authenticated':True,'status':s,**x,'eligible':bool(z),'preflight':p.locator('#sv-img-preflight').count()>0,'shown':sh,'saved':sa,'restored':re,'actionbar':p.locator('.iv-actionbar').count()>0,'overflow':p.evaluate('()=>Math.max(0,document.documentElement.scrollWidth-innerWidth)')}
def main():
 ch=chrome();sid=os.environ.get('SV_ADMIN_SESSION_ID','');sn=os.environ.get('SV_ADMIN_SESSION_NAME','PHPSESSID');r={'schema':2,'started_at':ts(),'login':{},'session_source':'ephemeral-audit-session','authenticated_profile':None,'home':{},'catalog':{},'image':{},'blocked_mutations':[],'errors':[]}
 if not sid or not sn:emit({**r,'overall':False,'failures':['ephemeral_session_missing'],'finished_at':ts()},8)
 with sync_playwright() as w:
  r['login']=login(w,ch);b=w.chromium.launch(executable_path=ch,headless=True);c=b.new_context(viewport={'width':390,'height':844},is_mobile=True,has_touch=True,locale='pt-BR',timezone_id='America/Sao_Paulo',service_workers="block");bl=[];guard(c,bl);c.add_cookies([{'name':sn,'value':sid,'url':B,'httpOnly':True,'secure':True,'sameSite':'Lax'}]);p=c.new_page();p.set_default_timeout(11000)
  try:
   h=home(p);r['home']=h
   if h.get('authenticated'):
    r['authenticated_profile']='ephemeral-audit-session';r['catalog']=catalog(p);r['image']=image(p)
   r['blocked_mutations']=bl[:20]
  except Exception as e:r['errors'].append(type(e).__name__)
  finally:c.close();b.close()
...
if __name__=='__main__':
 ...
 except Exception as e:emit({'schema':2,'overall':False,'failures':['top_exception'],'errors':[type(e).__name__],'finished_at':ts()},9)
