...
class RecoverOlistOauthFromBackupsTests(unittest.TestCase):
    ...
    def test_recovery_tries_newest_first_and_only_writes_provider_accepted_tuple(self) -> None:
        ...
        older = self.root / ".env.backup.100"
        newer = self.root / ".env.backup.200"
        write_env(
            older,
        ...
        self.assertEqual(seen, ["bad-backup-id", "good-id"])
        _, values = recovery.parse_env(current)
        self.assertEqual(values["UNCHANGED"], "keep-me")
        self.assertEqual(values["OLIST_CLIENT_ID"], "good-id")
        self.assertEqual(values["OLIST_CLIENT_SECRET"], "good-secret")
        self.assertEqual(values["OLIST_ACCESS_TOKEN"], "fresh-access")
        self.assertEqual(values["OLIST_REFRESH_TOKEN"], "fresh-refresh")
        self.assertTrue(list(self.root.glob(".env.pre-olist-recovery-*")))

    def test_recovery_does_not_modify_current_env_when_no_backup_is_accepted(self) -> None:
        current = self.root / ".env"
        original = (
        ...
        with self.assertRaisesRegex(recovery.RecoveryError, "no_backup_tuple_accepted_by_provider"):
    ...
    def test_incomplete_or_placeholder_backups_are_ignored(self) -> None:
        write_env(
